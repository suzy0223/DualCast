...
class TrafficStateDataset(AbstractDataset):
    ...
    def _load_geo(self):
        ...
        print("geo_to_ind = ",self.geo_to_ind)
    ...
    def _add_external_information_3d(self, df, ext_data=None):
        ...
        if self.add_time_in_day and not is_time_nan:
            ...
            print("time_in_day = ",time_in_day.shape)
        ...
        if self.add_day_in_week and not is_time_nan:
            ...
            is_holiday = day_in_week.argmax(axis=-1)
            is_holiday = np.expand_dims(is_holiday, axis=-1)
            is_holiday = (is_holiday+3)%7
            is_holiday = is_holiday.astype(np.int32)
            is_holiday[is_holiday >= 3] = 0
            is_holiday[is_holiday == 2] = 1
            holidays = [68]
            for i in range(len(holidays)):
                is_holiday[holidays[i]*48:(holidays[i]+1)*48] = 1
            # is_holiday[0*288:1*288] = 1
            # is_holiday[14*288:55*288] = 1
            # is_holiday[49*288:50*288] = 1
            # print("is_holiday = ",is_holiday.shape)
            # print(is_holiday)
            
            data_list.append(is_holiday)
        ...
        print(">>>data shap after adding time",data.shape)
        return data
