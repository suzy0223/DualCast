...
class TrafficStateExecutor(AbstractExecutor):
    def __init__(self, config, model):
        ...
        self.saved = self.config.get('saved_model', False)
    ...
    def save_model_with_epoch(self, epoch, model, optimizer):
        ensure_dir(self.cache_dir)
        config = dict()
        config['model_state_dict'] = model
        config['optimizer_state_dict'] = optimizer
        config['epoch'] = epoch
