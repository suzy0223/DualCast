...
class STSelfAttention(nn.Module):
    def __init__(
        ...
        self.geo_q2_conv = nn.Conv2d(dim, int(dim * self.geo_ratio), kernel_size=1, bias=qkv_bias)
        self.geo_k2_conv = nn.Conv2d(dim, int(dim * self.geo_ratio), kernel_size=1, bias=qkv_bias)
        self.geo_v2_conv = nn.Conv2d(dim, int(dim * self.geo_ratio), kernel_size=1, bias=qkv_bias)
        self.geo_q2_conv = nn.Conv2d(dim, int(dim * self.geo_ratio), kernel_size=1, bias=qkv_bias)
        self.geo_attn_drop2 = nn.Dropout(attn_drop)
    ...
    def forward(self, x, x_patterns, pattern_keys, adj=None, geo_mask=None, geo_mask2=None, sem_mask=None):
        B, T, N, D = x.shape
        ...
        # x_pattern = pattern_keys = (B, T, N, embed, output_dim)
        ...
        # B, T, self.geo_num_heads, N, self.head_dim
        ...
        hidden = geo_v*self.hopwise[0]

        # [K * B, T, N, d]
        B, T, k, N, d = geo_q.shape
        # Attention GA
        if self.global_att:
            teleM = torch.matmul(geo_k.transpose(3,4),geo_v)/N
            teleM = teleM.unsqueeze(-3)
            teleK = torch.sum(geo_k,dim=-2,keepdim=True)/N
            teleH = torch.einsum('bthnd,bthndz->bthnz',[geo_q,teleM])
            teleC = torch.einsum('bthnd,bthnd->bthn', [geo_q,teleK]).unsqueeze(-1)+self.cst

            teleH = teleH/teleC
            hidden += teleH*self.teleport
            # SA = F.softmax(geo_attn, dim=-1)
            # teleX = (SA @ geo_v).transpose(2, 3).reshape(B, T, N, int(D * self.geo_ratio))

        # hidden = (geo_v*self.hopwise[0]).transpose(2, 3).reshape(B, T, N, int(D * self.geo_ratio))
        ...
        row, col = edge_index
        deg = degree(col,2*N, dtype=geo_q.dtype)
        deg_inv = deg.pow(-1)
        deg_inv[deg_inv == float('inf')] = 0
        norm = deg_inv[row]
        

        # # To compute one-hop attention
        ...
        # print(num_step)
        for i in range(T):
            # [B, 1, 2N, D] -> [B, D, 1, 2N] -> [B, 1, 2N, D]
            key2 = self.geo_k2_conv((x2[:,i:i+2,:,:].reshape(B, 2*N, -1).unsqueeze(1)).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            geo_k2.append(key2)
            val2 = self.geo_v2_conv((x2[:,i:i+2,:,:].reshape(B, 2*N, -1).unsqueeze(1)).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            geo_v2.append(val2)
            query2 = self.geo_q2_conv((x2[:,i:i+2,:,:].reshape(B, 2*N, -1).unsqueeze(1)).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            geo_q2.append(query2)

        # [B, T, 2N, D] -> (B,T,2N,K,d) -> (B,T,K,2N,d)
        geo_k2 = torch.cat(geo_k2, dim=1).reshape(B, T, 2*N, self.geo_num_heads, self.head_dim).permute(0, 1, 3, 2, 4)
        geo_v2 = torch.cat(geo_v2, dim=1).reshape(B, T, 2*N, self.geo_num_heads, self.head_dim).permute(0, 1, 3, 2, 4)
        geo_q2 = torch.cat(geo_q2, dim=1).reshape(B, T, 2*N, self.geo_num_heads, self.head_dim).permute(0, 1, 3, 2, 4)
        
        geo_q2 = F.relu(geo_q2)
        geo_k2 = F.relu(geo_k2)

        M = torch.einsum('btknd,btknz->btkndz',[geo_k2,geo_v2])
        ...
        for hop in range(2):
            ...
            geo_k2 = scatter(K_j, col, dim=-2, reduce='sum')
            H = torch.einsum('btknd,btkndz->btknz',[geo_q2,M])
            C = torch.einsum('btknd,btknd->btkn',[geo_q2,geo_k2]).unsqueeze(-1) + self.cst
            H = H/C
        ...
        # (B,T,K,2N,d)
        # print(hidden.shape)
        hidden = hidden.transpose(2, 3).reshape(B, T, N, int(D * self.geo_ratio))
        
        # # [N,d] * [d,2N] -> [b,T,K, N, 2N]
        # geo_attn2 = (geo_q @ geo_k2.transpose(-2, -1)) * self.scale
        # # (N,2N)
        # mask2 = torch.cat((geo_mask,geo_mask2),dim=-1)
        # geo_attn2.masked_fill_(mask2, float('-inf'))
        # geo_attn2 = geo_attn2.softmax(dim=-1)
        # geo_attn2 = self.geo_attn_drop(geo_attn2)
        # geo_x2 = (geo_attn2 @ geo_v2).transpose(2, 3).reshape(B, T, N, int(D * self.geo_ratio))
        # hidden += geo_x2*self.hopwise[2]

        # if self.global_att:
        #     hidden += teleX*self.teleport
        ...
        x = self.proj(torch.cat([t_x, hidden, sem_x], dim=-1))
        x = self.proj_drop(x)
        # print(x)
        return x
...
class STEncoderBlock(nn.Module):
    ...
    def forward(self, x, x_patterns, pattern_keys, adj=None, geo_mask=None, geo_mask2=None, sem_mask=None):
        if self.type_ln == 'pre':
            x = x + self.drop_path(self.st_attn(self.norm1(x), x_patterns, pattern_keys, adj=adj, geo_mask=geo_mask, geo_mask2=geo_mask2, sem_mask=sem_mask))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        elif self.type_ln == 'post':
            x = self.norm1(x + self.drop_path(self.st_attn(x, x_patterns, pattern_keys, adj=adj, geo_mask=geo_mask, geo_mask2=geo_mask2, sem_mask=sem_mask)))
...
class PDFormer(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.num_prototypes = config.get('num_prototypes',17)
        ...
        self.slot_per_h = config.get("slot_per_h", 12)
        ...
        if self.type_short_path == "dist":
            ...
            # print("sh_mx",sh_mx)
            self.adj = (torch.from_numpy(self.adj_mx)+torch.eye(self.num_nodes)).to(self.device)
            sh_mx = sh_mx.T
            ...
            self.geo_mask2 = torch.zeros(self.num_nodes, self.num_nodes).to(self.device)
            s1 = torch.zeros(self.num_nodes, self.num_nodes).to(self.device)
            s2 = torch.zeros(self.num_nodes, self.num_nodes).to(self.device)
            s1[sh_mx >= self.far_mask_delta+1] = 1
            s2[sh_mx < self.far_mask_delta] = 1
            self.geo_mask2 = torch.logical_or(s1,s2).float().to(self.device)
            self.geo_mask2 = self.geo_mask2.bool()
        ...
        self.node_filter = nn.Linear(self.embed_dim, 2)
        self.anchors = Parameter(torch.rand(self.num_prototypes, self.output_window, self.num_nodes, self.skip_dim),requires_grad=True)
        self.teleport = Parameter(torch.ones(1, dtype=torch.float),requires_grad=True)


        enc_dpr_i = [x.item() for x in torch.linspace(0, drop_path, enc_depth)]
        self.encoder_blocks_i = nn.ModuleList([
            STEncoderBlock(
                ...
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop, drop_path=enc_dpr_i[i], act_layer=nn.GELU,
        ...
        self.end_conv1_i = nn.Conv2d(
            in_channels=self.input_window, out_channels=self.output_window, kernel_size=1, bias=True,
        )
        self.end_conv2_i= nn.Conv2d(
            in_channels=self.output_window, out_channels=self.output_dim, kernel_size=1, bias=True,
        )

        enc_dpr_e = [x.item() for x in torch.linspace(0, drop_path, enc_depth)]
        self.encoder_blocks_e = nn.ModuleList([
            STEncoderBlock(
                dim=self.embed_dim, s_attn_size=self.s_attn_size, t_attn_size=self.t_attn_size, geo_num_heads=geo_num_heads, sem_num_heads=sem_num_heads, t_num_heads=t_num_heads,
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop, drop_path=enc_dpr_e[i], act_layer=nn.GELU,
                norm_layer=partial(nn.LayerNorm, eps=1e-6), device=self.device, type_ln=type_ln, output_dim=self.output_dim,
            ) for i in range(enc_depth)
        ])

        self.skip_convs_e = nn.ModuleList([
            nn.Conv2d(
                in_channels=self.embed_dim, out_channels=self.skip_dim, kernel_size=1,
            ) for _ in range(enc_depth)
        ])

        self.end_conv1_e = nn.Conv2d(
            in_channels=self.input_window, out_channels=self.output_window, kernel_size=1, bias=True,
        )
        self.end_conv2_e = nn.Conv2d(
            in_channels=self.output_window, out_channels=self.output_dim, kernel_size=1, bias=True,
        )
        self.end_conv_skip_1 = nn.Conv2d(
            in_channels=self.skip_dim*2, out_channels=self.skip_dim, kernel_size=1, bias=True,
        )
        self.end_conv_skip_2 = nn.Conv2d(
    ...
    def forward(self, batch, lap_mx=None):
        ...
        # B, T, Nd
        timeofday = x[:,:,:,self.feature_dim-self.ext_dim]* 1440/(60/self.slot_per_h)
        timeofday = timeofday[...,0]
        dayofweek = x[:,:,:,self.feature_dim-self.ext_dim+1:self.feature_dim-self.ext_dim+8]

        dayofweek = dayofweek.argmax(dim=3)
        dayofweek = dayofweek[...,0]
        
        dayofweek = (dayofweek + 3)%7

        is_holiday = dayofweek.clone()
        ...
        x_patterns = torch.cat(x_pattern_list, dim=-1)  # (B, T, N, embed, output_dim)
        pattern_keys = torch.cat(pattern_key_list, dim=-1)

        enc = self.enc_embed_layer(x, lap_mx)
        atten = self.node_filter(enc)
        atten = torch.softmax(atten, dim=-1)
        atten_i = atten[:, :, :, 0]
        atten_e = atten[:, :, :, 1]
        enc_i = enc * torch.unsqueeze(atten_i, -1)
        enc_e = enc * torch.unsqueeze(atten_e, -1)

        skip_i = 0
        # print("enc_i",enc_i.shape)

        for i, encoder_block_i in enumerate(self.encoder_blocks_i):
            # B, T, N, D
            enc_i = encoder_block_i(enc_i, x_patterns, pattern_keys, self.adj, self.geo_mask, self.geo_mask2, self.sem_mask)
            # B, D, N, T - > B, D', N, T
            skip_i += self.skip_convs_i[i](enc_i.permute(0, 3, 2, 1))

        # print("skip_i",skip_i.shape)
        # B, D', N, T -> B, T, N, D' -> B, T, N, D'
        skip_i = self.end_conv1_i(F.relu(skip_i.permute(0, 3, 2, 1)))

        skip_e = 0
        # print("enc_e",enc_e.shape)
        for i, encoder_block_e in enumerate(self.encoder_blocks_e):
            # B, T, N, D
            enc_i = encoder_block_e(enc_e, x_patterns, pattern_keys, self.adj, self.geo_mask, self.geo_mask2, self.sem_mask)
            # B, D, N, T - > B, D', N, T
            skip_e += self.skip_convs_e[i](enc_e.permute(0, 3, 2, 1))

        # B, D', N, T -> B, T, N, D' -> B, T, N, D'
        skip_e = self.end_conv1_e(F.relu(skip_e.permute(0, 3, 2, 1)))

        p_Z = [[] for i in range(self.num_prototypes)]
        z = torch.zeros(skip_i.shape).to(skip_i.device)
        # 0-14; wokring day morning rush hour; working day evening rush hour; working day non-rush hour; Sat; Sun
        for i in range(B):
            if is_holiday[i, 0] == 1:
                # print(dayofweek[i, 1])
                if dayofweek[i, 0] == 1:
                    p_Z[-2].append(skip_i[i, ...]) # Sat
                    z[i] = self.anchors[-2]
                else:
                    p_Z[-1].append(skip_i[i, ...]) # Sun or public holiday
                    z[i] = self.anchors[-1]
            else:
                ...
                if timeofday[i, 0] >= 6*self.slot_per_h and timeofday[i, 0] < 9*self.slot_per_h:
                    p_Z[d*3].append(skip_i[i, ...])
                    z[i] = self.anchors[d*3]
                # evening rush hour: p2
                elif timeofday[i, 0] >= 16*self.slot_per_h and timeofday[i, 0] < 22*self.slot_per_h:
                    p_Z[d*3+1].append(skip_i[i, ...])
                    z[i] = self.anchors[d*3+1]
                else:
                    p_Z[d*3+2].append(skip_i[i, ...])
                    z[i] = self.anchors[d*3+2]

        # merge two branches by cat [B, T, N, D]
        skip = torch.cat((skip_i, skip_e), dim=-1)
        # B, T, N, 2D' -> B, D', N, T
        skip = self.end_conv_skip_1(F.relu(skip.permute(0, 3, 2, 1)))
        # output
        z = z.permute(0, 3, 2, 1)
        skip = skip + self.teleport * z
        # B, T, N, 2D' -> B,1 , N, T
        skip = self.end_conv_skip_2(F.relu(skip))
        skip = skip.permute(0, 3, 2, 1)

        # B, T, N, D' -> B, 1, N, D'
        g_feati = skip_i.clone()
        g_feati = self.end_conv2_i(F.relu(g_feati))
        # B, 1, N, D' -> B, D', N, 1
        ...
        # B, T, N, D' -> B, 1, N, D'
        g_feate = skip_e.clone()
        g_feate = self.end_conv2_e(F.relu(g_feate))
        # B, 1, N, D' -> B, D', N, 1
        ...
        return skip, g_feati, g_feate, p_Z, self.anchors
