...
def train(model, trainX, trainTE, trainY, valX, valTE, valY, mean, std, args, log, loss_criterion, kl_criterion, anchors, optimizer, scheduler, device):
    ...
    for epoch in range(args.max_epoch):
        ...
        for batch_idx in range(train_num_batch):
            ...
            optimizer.zero_grad()
            gfeati, gfeate, p_Z, anchors, pred = model(X, TE, is_train=True)
            ...
            if args.l_dbi:
                anchors = anchors.view(args.prototypes, -1)
            
                s = []
                for i in range(args.prototypes):
                    n = len(p_Z[i])
                    if n > 1:
                ...
                for i in range(args.prototypes):
                    if s[i] == 0:
                        ...
                        r = []
                        for j in range(args.prototypes):
                            if i != j and s[j] != 0:
        ...
        with torch.no_grad():
            for batch_idx in range(val_num_batch):
                ...
                _,_,_,_,pred = model(X, TE, is_train=True)
                pred = pred * std + mean
    ...
    return train_total_loss, val_total_loss, val_loss_min
